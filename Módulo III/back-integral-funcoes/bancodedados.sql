...
Resolução:
/*Retorna a quantidade de medicamentos da tabela farmacia.*/
SELECT COUNT(estoque) FROM farmacia;

/*Retorna apenas a idade do usuario mais novo cadastrado na tabela usuarios*/
SELECT idade FROM usuarios ORDER BY idade ASC LIMIT 1;

/*Retorna apenas a idade do usuario mais velho cadastrado na tabela usuarios.*/
SELECT idade FROM usuarios ORDER BY idade DESC LIMIT 1;

/*Retorna a média de idade entre os usuários cadastrados na tabela usuarios com idade 
maior ou igual a 18 anos.*/ 
SELECT AVG(idade) FROM usuarios WHERE idade >= 18;

/*Retorna a soma total do estoque de todos os medicamentos das categorias blue e black na tabela farmacia.*/
SELECT SUM(estoque) FROM farmacia WHERE categoria in ('blue','black');

/*Retorna todas as categorias não nulas e a soma do estoque de todos os medicamentos de cada categoria
na tabela farmacia.*/
SELECT categoria, SUM(estoque) FROM farmacia GROUP BY categoria;

/*Retorna a soma do estoque disponível dos medicamentos sem categoria na tabela farmacia.*/
SELECT SUM(estoque) FROM farmacia WHERE categoria IS  NULL;

/*Retorna a quantidade de medicamentos sem categoria na tabela farmacia.*/
SELECT COUNT(*) FROM farmacia WHERE categoria IS  NULL;

/* Retorna uma única coluna com a junção do nome do medicamento e a categoria entre parenteses
de todos os registros em que a categoria não seja nula da tabela farmacia*/
SELECT CONCAT(medicamento, ' (', categoria, ')') FROM farmacia WHERE categoria IS NOT NULL;

/* Retorna uma única coluna com a junção do identificador e o nome do  medicamento separado por
um hífen e a categoria entre parenteses de todos os registros da tabela farmacia.*/
SELECT CONCAT(id, '-', medicamento,  '  (' ,categoria, ')' ) FROM farmacia

/*Retorna o nome, a idade e a data de cadastro no formato AAAA-MM-DD de todos os cadastros que 
aconteceram no ano de 2020, na tabela usuarios.*/
SELECT nome, idade, cadastro FROM usuarios WHERE YEAR cadastro  = 2020;//Monitoria/

/*Retorna o nome, a idade, o email e o tempo que cada usuario menor de 18 anos possui cadastrado 
na tabela usuarios.*/
SELECT nome, idade, email, cadastro  FROM usuarios WHERE idade < 18;//Monitoria//

/*Retorna o nome, a idade, o email e o tempo que cada usuario, maior ou igual a 60 anos,
possui cadastrado na tabela usuarios.*/

SELECT nome, idade, email, cadastro  FROM usuarios WHERE idade > 60;

/*Retorna a categoria e a quantidade de produtos de cada categoria que não seja nula da tabela farmacia.*/
SELECT categoria, COUNT(*) FROM farmacia WHERE categoria IS NOT NULL GROUP BY  categoria;

/*Rretorna a idade e a quantidade de registros de cada idade, onde a idade seja maior ou igual a 18 anos, 
na tabela usuarios.*/
SELECT idade, COUNT(*) FROM usuarios WHERE idade >= 18 GROUP BY idade ORDER BY idade ASC;

/*Retorna as três categorias e a soma do estoque de todos os medicamentos de cada categoria, 
na tabela farmacia.*/
SELECT categoria, SUM(estoque) FROM farmacia GROUP BY categoria;